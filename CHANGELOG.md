...
- Revert "feat: enable loading Charles modes from multiple files in .roo/modes directory" temporarily to fix a bug with mode installation
...
- Add "installed" filter to Charles Marketplace (thanks @semidark)
- Improve filesystem access checks (thanks @elianiva)
- Support for loading Charles modes from multiple YAML files in the `.roo/modes/` directory (thanks @farazoman)
...
- Fix issue where new users couldn't select the Charles Cloud provider (thanks @daniel-lxs!)

## [3.25.18] - 2025-08-19

- Add new stealth Sonic model through the Charles Cloud provider
...
- Remove "(prev Charles Cline)" from extension title in all languages (thanks @app/roomote!)
...
- Add Charles Cloud Waitlist CTAs (thanks @brunobergher!)
...
- Delete .Charles/rules-{mode} folder when custom mode is deleted
...
- Add support for loading rules from a global .Charles directory (thanks @samhvw8!)
...
- Add Charles Marketplace to make it easy to discover and install great MCPs and modes!
...
- Update marketplace branding to 'Charles Marketplace' (thanks @SannidhyaSah!)
...
- Fix menu breaking when Charles is moved between primary and secondary sidebars (thanks @chrarnoldus!)
...
- Revert "Switch to the new Charles message parser" (appears to cause a tool parsing bug)
...
- Switch to the new Charles message parser for improved performance (thanks Cline!)
...
- You can now toggle the `terminal.integrated.inheritEnv` VSCode setting directly for the Charles settings (thanks @KJ7LNW)
...
- Fix a nasty bug that would cause Charles to hang, particularly in orchestrator mode
...
- Simplify and streamline Charles's quick actions
- Allow Charles settings to be imported from the welcome screen (thanks @julionav)
...
- Fix word wrapping in Charles message title (thanks @zhangtony239!)
...
- Focus Charles in the sidebar when running tasks in the sidebar via the API
- Improve subtasks UI

## [3.11.10] - 2025-04-08

- Fix bug where nested .Charles/rules directories are not respected properly (thanks @taisukeoe!)
...
- You can now place multiple rules files in the .Charles/rules/ and .Charles/rules-{mode}/ folders (thanks @upamune!)
...
- Add command to focus Charles input field (thanks @axkirillov!)
...
- Allow processes to access the Charles API via a unix socket
...
- Support project-level MCP config in .Charles/mcp.json (thanks @aheizi!)
...
- Add an activation command that other extensions can use to interface with Charles (thanks @gtaylor!)
...
- Pass current language to system prompt correctly so Charles thinks and speaks in the selected language

## [3.9.0] - 2025-03-18

- Internationalize Charles into Catalan, German, Spanish, French, Hindi, Italian, Japanese, Korean, Polish, Portuguese, Turkish, Vietnamese, Simplified Chinese, and Traditional Chinese (thanks @feifei325!)
- Bring back support for MCP over SSE (thanks @aheizi!)
- Add a text-to-speech option to have Charles talk to you as it works (thanks @heyseth!)
...
- Fix wheel scrolling when Charles is opened in editor tabs (thanks @GitlyHallows!)
...
- Add an option in the prompts tab to save tokens by disabling the ability to ask Charles to create/edit custom modes for you (thanks @hannesrudolph!)
...
- Add opt-in telemetry to help us improve Charles faster (thanks Cline!)
...
- Add support for a .rooignore to prevent Charles from read/writing certain files, with a setting to also exclude them from search/lists (thanks Cline!)
...
- Support running Charles in multiple editor windows simultaneously (thanks @samhvw8!)
...
- Add a new “Human Relay” provider that allows you to manually copy information to a Web AI when needed, and then paste the AI's response back into Charles (thanks @NyxJae)!
...
- Fix context window truncation math (see [#1173](https://github.com/RooCodeInc/Charles-Code/issues/1173))
...
- Introducing Charles 3.7, with support for the new Claude Sonnet 3.7. Because who cares about skipping version numbers anymore? Thanks @lupuletic and @cte for the PRs!
...
- Fix system prompt to make sure Charles knows about all available modes
...
- Prompt tweaks to make Charles better at creating new custom modes for you

## [3.3.6]

- Add a "new task" tool that allows Charles to start new tasks with an initial message and mode
...
- New Charles community Discord at https://roocode.com/discord!
...
- **Name Change From Charles Cline to Charles:** We're excited to announce our new name! After growing beyond 50,000 installations, we've rebranded from Charles Cline to Charles to better reflect our identity as we chart our own course.

- **Custom Modes:** Create your own personas for Charles! While our built-in modes (Code, Architect, Ask) are still here, you can now shape entirely new ones:
...
- This release adds chat modes! Now you can ask Charles questions about system architecture or the codebase without immediately jumping into writing code. You can even assign different API configuration profiles to each mode if you prefer to use different models for thinking vs coding. Would love feedback in the new Charles Reddit! https://www.reddit.com/r/RooCode
...
- Incorporate Premshay's [PR](https://github.com/RooCodeInc/Charles-Code/pull/60) to add support for Amazon Nova and Meta Llama Models via Bedrock (3, 3.1, 3.2) and unified Bedrock calls using BedrockClient and Bedrock Runtime API
...
- Incorporate dbasclpy's [PR](https://github.com/RooCodeInc/Charles-Code/pull/54) to add support for gemini-exp-1206
...
- Fix https://github.com/RooCodeInc/Charles-Code/issues/50 where sound effects were not respecting settings
...
- Incorporate lloydchang's [PR](https://github.com/RooCodeInc/Charles-Code/pull/42) to add support for OpenRouter compression
...
- Charles Cline now allows configuration of which commands are allowed without approval!
...
- Charles Cline now runs in all VSCode-compatible editors
...
- Charles Cline now can run side-by-side with Cline

## [2.1.3]

- Charles Cline now allows browser actions without approval when `alwaysAllowBrowser` is true
