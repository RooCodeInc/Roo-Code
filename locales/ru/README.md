...
0. **Откройте в Dev Container (рекомендуется):** После клонирования репозитория откройте его в VS Code и выполните «Reopen in Container», чтобы работать в предоставленной среде `.devcontainer`. Так вы получите нужные версии Node.js и pnpm без дополнительной ручной настройки.
