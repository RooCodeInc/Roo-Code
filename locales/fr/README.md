...
## Que peut faire Klaus Code pour VOUS ?
...
Klaus Code s'adapte à votre façon de travailler, pas l'inverse :
...
Il existe plusieurs façons d'exécuter l'extension Klaus Code :

### Mode développement (F5)

Pour un développement actif, utilisez le débogage intégré de VSCode :

Appuyez sur `F5` (ou allez dans **Exécuter** → **Démarrer le débogage**) dans VSCode. Cela ouvrira une nouvelle fenêtre VSCode avec l'extension Klaus Code en cours d'exécution.
