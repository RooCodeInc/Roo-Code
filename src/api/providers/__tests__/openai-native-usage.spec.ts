...
describe("OpenAiNativeHandler - normalizeUsage", () => {
	...
	describe("formatFullConversation robustness", () => {
		let handler: OpenAiNativeHandler
		...
		it("should skip undefined and null messages safely", () => {
			const messages: any[] = [
				undefined,
				null,
				{
			...
			const result = (handler as any).formatFullConversation("", messages)
		...
		it("should skip malformed content blocks when formatting", () => {
			const messages: any[] = [
				{
					role: "user",
					content: [
						undefined,
						{ type: "text", text: "ok" },
						// malformed block without a type should be ignored
						{ notype: true },
					],
				},
			]
