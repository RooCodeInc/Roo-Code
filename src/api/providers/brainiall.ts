...
import { type BrainiallModelId, brainiallDefaultModelId, brainiallModels, ApiProviderError } from "@roo-code/types"
...
const BRAINIALL_DEFAULT_TEMPERATURE = 0

export class BrainiallHandler extends BaseProvider implements SingleCompletionHandler {
	protected options: ApiHandlerOptions
	private client: OpenAI
	private readonly providerName = "Brainiall"

	constructor(options: ApiHandlerOptions) {
		super()
		this.options = options

		const apiKey = this.options.brainiallApiKey ?? "not-provided"
	...
	override getModel() {
		const id =
			this.options.apiModelId && this.options.apiModelId in brainiallModels
				? (this.options.apiModelId as BrainiallModelId)
				: brainiallDefaultModelId

		const info = brainiallModels[id]
		const params = getModelParams({
			format: "openai",
			modelId: id,
			model: info,
			settings: this.options,
			defaultTemperature: BRAINIALL_DEFAULT_TEMPERATURE,
		})
	...
	override async *createMessage(
		...
		const requestOptions = {
			...
			temperature: this.options.modelTemperature ?? BRAINIALL_DEFAULT_TEMPERATURE,
			messages: [
