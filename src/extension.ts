...
/**
 * Check if we should auto-open the s2sCopilot sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening s2sCopilot sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the s2sCopilot sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
