// npx vitest core/prompts/__tests__/responses-jabberwockignore.spec.ts

import type { Mock } from "vitest"

import { formatResponse } from "../responses"
import { JabberwockIgnoreController, LOCK_TEXT_SYMBOL } from "../../ignore/JabberwockIgnoreController"
...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.jabberwockIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.jabberwockIgnoreError("secrets/api-keys.json")
			...
			expect(parsed.suggestion).toContain("update the .jabberwockignore file")
		})
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.jabberwockIgnoreError(testPath)
				const parsed = JSON.parse(errorMessage) as any
	...
	describe("formatResponse.formatFilesList with JabberwockIgnoreController", () => {
		/**
		 * Tests file listing with rooignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new JabberwockIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList when showJabberwockIgnoredFiles is set to false
		 */
		it("should hide ignored files when showJabberwockIgnoredFiles is false", async () => {
			// Create controller
			const controller = new JabberwockIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showJabberwockIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showJabberwockIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with JabberwockIgnoreController
		 */
		it("should handle truncation with JabberwockIgnoreController", async () => {
			// Create controller
			const controller = new JabberwockIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with JabberwockIgnoreController", async () => {
			// Create controller
			const controller = new JabberwockIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .jabberwockignore instructions for the LLM", async () => {
			// Create controller
			const controller = new JabberwockIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .jabberwockignore exists", async () => {
			// Set up no .jabberwockignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .jabberwockignore
			const controller = new JabberwockIgnoreController(TEST_CWD)
			await controller.initialize()
