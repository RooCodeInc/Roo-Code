...
export function getToolUseGuidelinesSection(
	...
	if (allowMultiToolCalls) {
		guidelinesList.push(
			`${itemNumber++}. If multiple actions are needed, you may call multiple tools in a single message (including different tool types) when it is efficient and logically grouped. Proactively batch low-risk information gathering (codebase_search, search_files, list_files, list_code_definition_names, read_file, etc.) together so you collect every required snippet before deciding what to change. When you already know several files or directories must be inspected, enumerate them and issue the corresponding tool blocks back-to-back so the user receives one consolidated request. Reserve sequential, one-tool-per-message behavior for actions whose parameters depend on earlier results or that affect external state (execute_command, browser_action, use_mcp_tool for remote calls, write_to_file, apply_diff, insert_content, generate_image, etc.). For editing workflows, gather context via batched reads first, then send a separate message that issues the editing tools using the confirmed context. After finishing any batched set of tools, always wait for the user response before continuing.`,
		)
	} else {
		guidelinesList.push(
