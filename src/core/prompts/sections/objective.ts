...
4. Once you've completed the user's task, you must use the attempt_completion tool to present the result of the task to the user. A task is considered complete ONLY when ALL of the following are true:
   ...
   c) If the task required running commands, they have completed successfully
   d) No errors, warnings, or "TODO" items remain that are part of the task scope
   e) The functionality works as intended (not just "partially works" or "mostly done")
   f) You have verified the changes solve the original problem completely
