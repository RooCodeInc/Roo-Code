...
/**
 * Returns a shell-specific note about command chaining syntax for different platforms.
 */
function getCommandChainNote(): string {
	...
	if (shell.includes("powershell") || shell.includes("pwsh")) {
		return "Note: Using `;` for PowerShell command chaining. For bash/zsh use `&&`, for cmd.exe use `&`."
	}

	// Check for cmd.exe
	if (shell.includes("cmd.exe")) {
		return "Note: Using `&` for cmd.exe command chaining. For bash/zsh use `&&`, for PowerShell use `;`."
	}
