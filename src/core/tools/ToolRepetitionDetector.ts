...
export class ToolRepetitionDetector {
	...
	constructor(consecutiveLimit: number = 3, historyLength: number = 20, patternLimit: number = 2) {
		this.consecutiveIdenticalToolCallLimit = consecutiveLimit
		this.historyMaxLength = historyLength
		this.patternRepetitionLimit = patternLimit
	}
	...
	public check(currentToolCallBlock: ToolUse): {
		...
		let isConsecutiveRepetition = false
		if (this.previousToolCallJson === currentToolCallJson) {
			this.consecutiveIdenticalToolCallCount++
			isConsecutiveRepetition = this.consecutiveIdenticalToolCallCount >= this.consecutiveIdenticalToolCallLimit
		} else {
		...
		if (isConsecutiveRepetition || patternRepetition) {
	...
	private detectPatternRepetition(): boolean {
		...
		for (let patternLength = 1; patternLength <= maxPatternLength; patternLength++) {
			...
			if (this.hasRepeatingPattern(historyArray, patternLength)) {
				return true
			}
		}
	...
	private hasRepeatingPattern(history: string[], patternLength: number): boolean {
		...
		while (position >= 0) {
			...
			for (let i = 0; i < patternLength; i++) {
				if (history[position - i] !== recentPattern[patternLength - 1 - i]) {
					isMatch = false
					break
				}
			}
