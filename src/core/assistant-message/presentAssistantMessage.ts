...
import { hookEngine, type HookPreToolUseContext } from "../../hooks/HookEngine"
...
export async function presentAssistantMessage(cline: Task) {
	...
	switch (block.type) {
		...
		case "tool_use": {
			...
			let hookContext: HookPreToolUseContext | undefined
			if (!block.partial) {
				const preToolUseResult = await hookEngine.preToolUse(cline, block)
				if (!preToolUseResult.allowExecution) {
					const errorMessage = preToolUseResult.errorMessage || `PreToolUse denied tool ${block.name}`
					cline.consecutiveMistakeCount++
				...
				hookContext = preToolUseResult.context
			}

			const didToolFailBeforeExecution = cline.didToolFailInCurrentTurn
			...
			try {
				...
				if (!block.partial && hookContext) {
					const didFailInThisDispatch = cline.didToolFailInCurrentTurn && !didToolFailBeforeExecution
					const executionSucceeded = !dispatchError && !didFailInThisDispatch
					await hookEngine.postToolUse(cline, block, hookContext, executionSucceeded)
				}
			}
