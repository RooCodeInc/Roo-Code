...
/**
 * Controls write access to Jabberwock configuration files by enforcing protection patterns.
 * Prevents auto-approved modifications to sensitive Jabberwock configuration files.
 */
export class JabberwockProtectedController {
	private cwd: string
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(JabberwockProtectedController.PROTECTED_PATTERNS)
	}
	...
	getProtectionMessage(): string {
		return "This is a Jabberwock configuration file and requires approval for modifications"
	}
	...
	getInstructions(): string {
		const patterns = JabberwockProtectedController.PROTECTED_PATTERNS.join(", ")
	...
	static getProtectedPatterns(): readonly string[] {
		return JabberwockProtectedController.PROTECTED_PATTERNS
	}
}
