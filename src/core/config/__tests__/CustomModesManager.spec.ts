...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = path.join(mockWorkspacePath, ".jabberwockmodes")
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .jabberwockmodes file and JSON for global customModes", async () => {
		...
		it("should merge modes with .jabberwockmodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .jabberwockmodes since it takes precedence
		...
		it("should handle missing .jabberwockmodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .jabberwockmodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .jabberwockmodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .jabberwockmodes precedence", async () => {
			...
			// Should update global state with merged modes where .jabberwockmodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .jabberwockmodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .jabberwockmodes file when adding project-specific mode", async () => {
			...
			// Mock .jabberwockmodes to not exist initially
			...
			// Verify .jabberwockmodes was created with the project mode
			...
			// Verify the content written to .jabberwockmodes
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".jabberwockmodes"),
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .jabberwockmodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				// Verify that no files were written outside the .jabberwock directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".jabberwockmodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".jabberwock"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(
					expect.stringContaining(path.join(".jabberwock", "rules-test-mode")),
					{
						recursive: true,
						force: true,
					},
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .jabberwockmodes file", async () => {
		...
		it("should return false when .jabberwockmodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.jabberwock/rules-test-mode" },
			])
		...
		it("should work with global custom modes when .jabberwockmodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .jabberwockmodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .jabberwockmodes", async () => {
			const roomodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .jabberwockmodes", async () => {
			const roomodesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.jabberwock/rules-code" },
			])
		...
		it("should successfully export global mode with rules from global .jabberwock directory", async () => {
			// Mock a global mode
			const globalMode = {
