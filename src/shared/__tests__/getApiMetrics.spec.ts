...
describe("getApiMetrics", () => {
	...
	describe("Basic functionality", () => {
		it("should calculate metrics from a single api_req_started message", () => {
			...
			expect(result.contextTokens).toBe(100) // tokensIn only (input tokens)
		})
		...
		it("should calculate metrics from multiple api_req_started messages", () => {
			...
			expect(result.contextTokens).toBe(50) // tokensIn from last message
		})
	...
	describe("Edge cases", () => {
		...
		it("should handle partial metrics in api_req_started message", () => {
			...
			// The implementation will use the last message that has any usable tokens
			// In this case, it's the message with tokensOut:200 (since the last few messages have no tokensIn)
			expect(result.contextTokens).toBe(200) // fallback to tokensOut
		})
		...
		it("should handle non-number values in api_req_started message", () => {
			...
			// Non-number values should be ignored for contextTokens
			expect(result.contextTokens).toBe(0)
		})
	})
	...
	describe("Context tokens calculation", () => {
		it("should calculate contextTokens from the last api_req_started message", () => {
			...
			// Should use the input tokens from the last api_req_started message
		...
		it("should handle missing values when calculating contextTokens", () => {
			...
			expect(result.contextTokens).toBe(0) // missing/invalid input tokens
