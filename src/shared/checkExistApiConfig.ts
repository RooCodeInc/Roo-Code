...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	// Special case for fake-ai, qwen-code, and roo providers which don't need any configuration.
	if (config.apiProvider && ["fake-ai", "qwen-code", "roo"].includes(config.apiProvider)) {
		return true
	}
