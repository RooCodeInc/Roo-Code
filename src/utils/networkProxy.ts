 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the Klaus Code extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure Klaus Code's `fetch()` calls are proxied, patch global fetch in debug mode.
