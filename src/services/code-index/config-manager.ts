...
export class CodeIndexConfigManager {
	...
	/**
	 * Apply external configuration (e.g., from API endpoint)
	 * This method updates the configuration and triggers a reload if necessary
	 */
	public async applyExternalConfiguration(externalConfig: {
	...
	}): Promise<{ requiresRestart: boolean; appliedSettings: string[] }> {
		const appliedSettings: string[] = []

		// Get current configuration snapshot before applying changes
		...
		const currentGlobalConfig = this.contextProxy?.getGlobalState("codebaseIndexConfig") ?? {}
		const updatedGlobalConfig = { ...currentGlobalConfig }

		if (externalConfig.codebaseIndexEnabled !== undefined) {
			updatedGlobalConfig.codebaseIndexEnabled = externalConfig.codebaseIndexEnabled
			appliedSettings.push("codebaseIndexEnabled")
		}

		if (externalConfig.codebaseIndexQdrantUrl !== undefined) {
			updatedGlobalConfig.codebaseIndexQdrantUrl = externalConfig.codebaseIndexQdrantUrl
			appliedSettings.push("codebaseIndexQdrantUrl")
		}

		if (externalConfig.codebaseIndexEmbedderProvider !== undefined) {
			updatedGlobalConfig.codebaseIndexEmbedderProvider = externalConfig.codebaseIndexEmbedderProvider
			appliedSettings.push("codebaseIndexEmbedderProvider")
		}

		if (externalConfig.codebaseIndexEmbedderBaseUrl !== undefined) {
			updatedGlobalConfig.codebaseIndexEmbedderBaseUrl = externalConfig.codebaseIndexEmbedderBaseUrl
			appliedSettings.push("codebaseIndexEmbedderBaseUrl")
		}

		if (externalConfig.codebaseIndexEmbedderModelId !== undefined) {
			updatedGlobalConfig.codebaseIndexEmbedderModelId = externalConfig.codebaseIndexEmbedderModelId
			appliedSettings.push("codebaseIndexEmbedderModelId")
		}

		if (externalConfig.codebaseIndexOpenAiCompatibleBaseUrl !== undefined) {
			updatedGlobalConfig.codebaseIndexOpenAiCompatibleBaseUrl =
				externalConfig.codebaseIndexOpenAiCompatibleBaseUrl
			appliedSettings.push("codebaseIndexOpenAiCompatibleBaseUrl")
		}

		// Update global state
		await this.contextProxy?.updateGlobalState("codebaseIndexConfig", updatedGlobalConfig)

		// Update secrets if provided
		const secretUpdates: Array<() => Promise<void>> = []

		if (externalConfig.openAiKey !== undefined) {
			secretUpdates.push(async () => {
				if (this.contextProxy) {
					await this.contextProxy.storeSecret("codeIndexOpenAiKey", externalConfig.openAiKey!)
				}
			})
			appliedSettings.push("codeIndexOpenAiKey")
		}

		if (externalConfig.qdrantApiKey !== undefined) {
			secretUpdates.push(async () => {
				if (this.contextProxy) {
					await this.contextProxy.storeSecret("codeIndexQdrantApiKey", externalConfig.qdrantApiKey!)
				}
			})
			appliedSettings.push("codeIndexQdrantApiKey")
		}

		if (externalConfig.openAiCompatibleApiKey !== undefined) {
			secretUpdates.push(async () => {
				if (this.contextProxy) {
					await this.contextProxy.storeSecret(
						"codebaseIndexOpenAiCompatibleApiKey",
						externalConfig.openAiCompatibleApiKey!,
					)
				}
			})
			appliedSettings.push("codebaseIndexOpenAiCompatibleApiKey")
		}

		if (externalConfig.geminiApiKey !== undefined) {
			secretUpdates.push(async () => {
				if (this.contextProxy) {
					await this.contextProxy.storeSecret("codebaseIndexGeminiApiKey", externalConfig.geminiApiKey!)
				}
			})
			appliedSettings.push("codebaseIndexGeminiApiKey")
		}

		if (externalConfig.mistralApiKey !== undefined) {
			secretUpdates.push(async () => {
				if (this.contextProxy) {
					await this.contextProxy.storeSecret("codebaseIndexMistralApiKey", externalConfig.mistralApiKey!)
				}
			})
			appliedSettings.push("codebaseIndexMistralApiKey")
		}

		await Promise.all(secretUpdates.map((update) => update()))

		// Refresh secrets to ensure they're loaded
		await this.contextProxy?.refreshSecrets()

		// Reload configuration to update instance variables
		await this.loadConfiguration()

		// Check if restart is required
		const requiresRestart = this.doesConfigChangeRequireRestart(previousConfig)
