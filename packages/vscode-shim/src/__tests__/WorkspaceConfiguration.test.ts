...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("jabberwock", context)
			...
			setRuntimeConfig("jabberwock", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("jabberwock", "testSetting", "testValue")

			expect(getRuntimeConfig("jabberwock.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("jabberwock", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("jabberwock.setting1")).toBe("value1")
			expect(getRuntimeConfig("jabberwock.setting2")).toBe(42)
			expect(getRuntimeConfig("jabberwock.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("jabberwock.defined")).toBe("value")
			expect(getRuntimeConfig("jabberwock.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("jabberwock", "setting1", "value1")
			setRuntimeConfig("jabberwock", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("jabberwock.setting1")).toBeUndefined()
			expect(getRuntimeConfig("jabberwock.setting2")).toBeUndefined()
		})
