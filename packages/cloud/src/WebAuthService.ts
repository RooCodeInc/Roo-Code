...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			this.log(`[auth] Error initiating Klaus Code Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate Klaus Code Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from Klaus Code Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with Klaus Code Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid Klaus Code Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with Klaus Code Cloud")
			}

			this.log("[auth] Successfully authenticated with Klaus Code Cloud")
		} catch (error) {
			this.log(`[auth] Error handling Klaus Code Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle Klaus Code Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from Klaus Code Cloud")
			}

			this.log("[auth] Logged out from Klaus Code Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from Klaus Code Cloud: ${error}`)
			throw new Error(`Failed to log out from Klaus Code Cloud: ${error}`)
		}
	}
