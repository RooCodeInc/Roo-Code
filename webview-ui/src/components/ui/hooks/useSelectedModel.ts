...
function getSelectedModel({
	...
	const { id, info: defaultInfo } = ((): { id: string; info: ModelInfo | undefined } => {
		switch (provider) {
			case "openrouter": {
	...
	const customInfo = apiConfiguration.customModelInfo

	// Merge customInfo with defaultInfo if customInfo exists
	const finalInfo = customInfo ? { ...defaultInfo, ...customInfo } : defaultInfo

	return { id, info: finalInfo }
}
