	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"cloud:signIn": "Connect to Klaus Code Cloud",
				"cloud:cloudBenefitsTitle": "Try Klaus Code Cloud",
				...
				"cloud:visitCloudWebsite": "Visit Klaus Code Cloud",
				"cloud:taskSync": "Task sync",
				"cloud:taskSyncDescription": "Sync your tasks for viewing and sharing on Klaus Code Cloud",
				...
				"cloud:remoteControlDescription":
					"Enable following and interacting with tasks in this workspace with Klaus Code Cloud",
				...
				"cloud:cloudUrlPillLabel": "Klaus Code Cloud URL: ",
			}
...
describe("CloudView", () => {
	it("should display benefits when user is not authenticated", () => {
		render(<CloudView userInfo={null} isAuthenticated={false} cloudApiUrl="https://app.tbd" />)

		// Check that the benefits section is displayed
		expect(screen.getByRole("heading", { name: "Try Klaus Code Cloud" })).toBeInTheDocument()
	...
	it("should not display benefits when user is authenticated", () => {
		...
		render(<CloudView userInfo={mockUserInfo} isAuthenticated={true} cloudApiUrl="https://app.tbd" />)
	...
	it("should display remote control toggle when user has extension bridge enabled and roomote control enabled", () => {
		...
		expect(
			screen.getByText("Enable following and interacting with tasks in this workspace with Klaus Code Cloud"),
		).toBeInTheDocument()
	})
	...
	it("should not display cloud URL pill when pointing to production", () => {
		...
		expect(screen.queryByText(/Klaus Code Cloud URL:/)).not.toBeInTheDocument()
	})
	...
	it("should display cloud URL pill when pointing to non-production environment", () => {
		...
		expect(screen.getByText(/Klaus Code Cloud URL:/)).toBeInTheDocument()
